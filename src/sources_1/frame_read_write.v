...
afifo_16i_64o_512 write_buf(
    .wr_clk(write_clk),
    .wr_rst(write_fifo_aclr),		//复位
    .wr_en(write_en),				//写使能 来自8转16输出
    .wr_data(write_data),			//输入的16bit数据 来自8转16
    .wr_full(),
    .wr_water_level(),
    .almost_full(),
    .rd_clk(mem_clk),
    .rd_rst(write_fifo_aclr),
    .rd_en(wr_burst_data_req),		//数据请求，来自axi控制器输出
    .rd_data(wr_burst_data),		//64位数据输出,给axi控制器
    .rd_empty(),
    .rd_water_level(rdusedw[8:0]),	//输出9位读水位  给fifo write模块
    .almost_empty());
/***************************FIFO控制******************/
frame_fifo_write
#
...
(  
	.rst                        (rst                      ),
	.mem_clk                    (mem_clk                  ),
	.wr_burst_req               (wr_burst_req             ),	//传输请求 给axi控制器
	.wr_burst_len               (wr_burst_len             ),	//10位传输长度 给axi控制器
	.wr_burst_addr              (wr_burst_addr            ),	 //28位传输地址 给axi控制器
	.wr_burst_data_req          (wr_burst_data_req        ),	//数据请求，来自axi控制器输出
	.wr_burst_finish            (wr_burst_finish          ),	//传输完成 来自axi控制器输出
	.write_req                  (write_req                ),	//写数据请求 来自摄像头数据写请求模块
	.write_req_ack              (write_req_ack            ),	//写数据响应 给摄像头数据写请求模块
	.write_finish               (write_finish             ),	//无
	.write_addr_0               (write_addr_0             ),
	.write_addr_1               (write_addr_1             ),
	.write_addr_2               (write_addr_2             ),
	.write_addr_3               (write_addr_3             ),
	.write_addr_index           (write_addr_index         ),    //地址选择 来自摄像头数据写请求模块
	.write_len                  (write_len                ),   //28位写数据长度 每张图的字节数/8
	.fifo_aclr                  (write_fifo_aclr          ),	//给fifo的复位信号
	.rdusedw                    ({7'h0,rdusedw[8:0]}                 ) //输出9位读水位 来自fifo
